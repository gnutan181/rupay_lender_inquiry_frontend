...
const CorrectionPanCard = () => {

  const navigate = useNavigate();
  const [correctionPanCard, setCorrectionPanCard] = useState([]);
  ...
  const fetchData = async () => {
    const { data } = await axiosInstance.get("/ca/correction-Pan");
    setCorrectionPanCard(data.details);
  ...
  useEffect(() => {
    if (correctionPanCard) {
      setItems(correctionPanCard);
      setPageCount(Math.ceil(correctionPanCard.length / itemsPerPage));
    }
  }, [correctionPanCard]);
  ...
  const viewServiceDetails = (serviceItemId)=>{
    navigate(`/ca-service-details/Correction-Pan/${serviceItemId}`)
  }
...
export default CorrectionPanCard;
