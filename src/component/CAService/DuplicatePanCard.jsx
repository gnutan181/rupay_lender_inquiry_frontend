...
const DuplicatePanCard = () => {

  const navigate = useNavigate();
  const [duplicatePanCard, setDuplicatePanCard] = useState([]);
  ...
  const fetchData = async () => {
    const { data } = await axiosInstance.get("/ca/duplicate-Pan");

    // adjuct this one
    setDuplicatePanCard(data.details);
  ...
  useEffect(() => {
    if (duplicatePanCard) {
      setItems(duplicatePanCard);
      setPageCount(Math.ceil(duplicatePanCard.length / itemsPerPage));
    }
  }, [duplicatePanCard]);
  ...
  const viewServiceDetails = (serviceItemId)=>{
    navigate(`/ca-service-details/Duplicate-Pan/${serviceItemId}`)
  }
...
export default DuplicatePanCard;
