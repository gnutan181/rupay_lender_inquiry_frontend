...
const Msme = () => {
  ...
  const fetchData = async () => {
    const { data } = await axiosInstance.get("/ca/msme");

    // adjuct this one
    setMsme(data.details);
  ...
  useEffect(() => {
    if (msme) {
      setItems(msme);
      setPageCount(Math.ceil(msme.length / itemsPerPage));
    }
  }, [msme]);
  ...
  const viewServiceDetails = (serviceItemId)=>{
    navigate(`/ca-service-details/MSME/${serviceItemId}`)
  }
