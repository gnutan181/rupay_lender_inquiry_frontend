...
const GstRegistration = () => {
  ...
  const fetchData = async () => {
    const { data } = await axiosInstance.get("/ca/gst");
    setGstRegistration(data.details);
  ...
  useEffect(() => {
    if (gstRegistration) {
      setItems(gstRegistration);
      setPageCount(Math.ceil(gstRegistration.length / itemsPerPage));
    }
  }, [gstRegistration]);
...
const navigate = useNavigate()
  const viewServiceDetails = (serviceItemId)=>{
    navigate(`/ca-service-details/GST-Registration/${serviceItemId}`)
  }
