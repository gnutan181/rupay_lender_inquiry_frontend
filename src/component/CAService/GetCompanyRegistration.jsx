...
const GetCompanyRegistration = () => {
  ...
  const fetchData = async () => {
    const { data } = await axiosInstance.get("/ca/business-Registration");
    setCompanyRegistration(data.details);
  ...
  useEffect(() => {
    if (companyRegistration) {
      setItems(companyRegistration);
      setPageCount(Math.ceil(companyRegistration.length / itemsPerPage));
    }
  }, [companyRegistration]);
  ...
  const viewServiceDetails = (serviceItemId)=>{
    navigate(`/ca-service-details/Company-Registration/${serviceItemId}`)
  }
...
export default GetCompanyRegistration;
