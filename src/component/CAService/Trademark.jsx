...
const Trademark = () => {
  ...
  const fetchData = async () => {
    ...
    const { data } = await axiosInstance.get("/ca/trademark");

    // adjuct this one
    setTrademark(data.details);
  ...
  useEffect(() => {
    if (trademark) {
      setItems(trademark);
      setPageCount(Math.ceil(trademark.length / itemsPerPage));
    }
  }, [trademark]);
  ...
  const viewServiceDetails = (serviceItemId)=>{
    navigate(`/ca-service-details/Trademark/${serviceItemId}`)
  }
