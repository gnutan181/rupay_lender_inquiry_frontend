...
const FoodLicense = () => {
  ...
  const fetchData = async () => {
    const { data } = await axiosInstance.get("/ca/food-License");

    // adjuct this one
    setFoodLicense(data.details);
  ...
  useEffect(() => {
    if (foodLicense) {
      setItems(foodLicense);
      setPageCount(Math.ceil(foodLicense.length / itemsPerPage));
    }
  }, [foodLicense]);
  ...
  const viewServiceDetails = (serviceItemId)=>{
    navigate(`/ca-service-details/Food-License/${serviceItemId}`)
  }
                    ...
                    <tbody key={i} className="divide-y divide-gray-200">
                      <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 text-[#3B3935] font-normal text-xs md:text-sm">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                          {item?.applicantDetails?.name || "N/A"}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                          {item?.personOrBusinessName || "N/A"}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                          {item?.email || "N/A"}
                        </td>
